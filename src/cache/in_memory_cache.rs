...
pub trait Cache: Send + Sync {
    ...
    async fn get<T: for<'a> Deserialize<'a> + Send>(&self, key: &str) -> Result<Option<T>, SplitwiseError>;

    /// Set a value in the cache with an optional TTL (in seconds)
    async fn set<T: Serialize + Send>(&self, key: &str, value: &T, ttl: Option<u64>) -> Result<(), SplitwiseError>;
